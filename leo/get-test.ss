(import (micascheme) (get) (leo get))

(check-gets word?-getter "" #f 0)
(check-gets word?-getter "(foo)" #f 0)
(check-gets word?-getter "foo" 'foo 3)
(check-gets word?-getter "foo(" 'foo 3)
(check-gets word?-getter "foo bar" 'foo 3)

(check-gets atom-getter "" (eof-object) 0)
(check-gets atom-getter "foo-bar! bar" 'foo-bar! 8)
(check-gets atom-getter "foo bar" 'foo 3)
(check-gets atom-getter "foo: bar" 'foo: 4)
(check-gets atom-getter "123 bar" 123 3)
(check-gets atom-getter "+123 bar" +123 4)
(check-gets atom-getter "-123 bar" -123 4)
(check-gets atom-getter "3.14 bar" 3.14 4)
(check-gets atom-getter "\"foo\" bar" "foo" 5)

(check-get-raises atom-getter " foo bar")
(check-get-raises atom-getter "() bar")
(check-get-raises atom-getter "(foo) bar")
(check-get-raises atom-getter "#t bar")

(check-gets line-getter "" (eof-object) 0)
(check-gets line-getter "123" 123 3)
(check-gets line-getter "\"foo\"" "foo" 5)
(check-gets line-getter "foo" 'foo 3)
(check-gets line-getter "foo bar" '(foo bar) 7)
(check-gets line-getter "foo bar goo" '(foo (bar goo)) 11)

(check-get-raises line-getter "123 ")
