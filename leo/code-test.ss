...
(check-code=? (line-code '(foo . bar)) "foo and bar")
(check-code=? (line-code '(foo gar . bar)) "foo (gar, and bar)")
(check-code=? (line-code '(foo)) "foo ()")
(check-code=? (line-code '(foo bar)) "foo bar")
(check-code=? (line-code '((foo bar))) "list foo bar")
...
(check-code=? (line-code (box '(foo . bar))) "box foo and bar")
...
(check-code=?
  ...
  "vector (null, true, 123, char a, \"foo\", foo, foo bar, foo and bar)")
...
(check-block-code '((x 10 20) (y 30 40)) "list" "  x: 10, 20" "  y: 30, 40")
