  ...
  (define (expand-expr $env $expr)
    (syntax-case $expr
      ...
      ((- a)
        (lets
          ($a (expand-expr $env #'a))
          (if (number? (syntax->datum $a))
            (datum->syntax #'- (- (syntax->datum $a)))
            #`(- #,$a))))
      ((- a b)
        (lets
          ($a (expand-expr $env #'a))
      ...
      ((div a b)
        (lets
          ...
          ($b (expand-expr $env #'b))
          (if (and (number? (syntax->datum $a)) (number? (syntax->datum $b)))
            (datum->syntax #'div (div (syntax->datum $a) (syntax->datum $b)))
            #`(div #,$a #,$b))))
      ((and)
        #'#t)
      ((and a)
      ...
      ((and a b)
        (lets
          ($a (expand-expr $env #'a))
          ($b (expand-expr $env #'b))
          (if (and (boolean? (syntax->datum $a)) (boolean? (syntax->datum $b)))
            (datum->syntax #'and (and (syntax->datum $a) (syntax->datum $b)))
            #`(and #,$a #,$b))))
      ((and a b c cs ...)
      ...
      ((or a b)
        (lets
          ...
          ($b (expand-expr $env #'b))
          (if (and (boolean? (syntax->datum $a)) (boolean? (syntax->datum $b)))
            (datum->syntax #'or (or (syntax->datum $a) (syntax->datum $b)))
            #`(or #,$a #,$b))))
      ((or a b c cs ...)
      ...
      ((not a)
        (lets
          ($a (expand-expr $env #'a))
          (if (boolean? (syntax->datum $a))
            (datum->syntax #'not (not (syntax->datum $a)))
            #`(not #,$a))))
      ((bitwise-and)
        #'-1)
      ((bitwise-and a)
      ...
      ((bitwise-and a b)
        (lets
          ($a (expand-expr $env #'a))
          ($b (expand-expr $env #'b))
          (if (and (integer? (syntax->datum $a)) (integer? (syntax->datum $b)))
            (datum->syntax #'bitwise-and (bitwise-and (syntax->datum $a) (syntax->datum $b)))
            #`(bitwise-and #,$a #,$b))))
      ...
      ((bitwise-ior a b)
        (lets
          ...
          ($b (expand-expr $env #'b))
          (if (and (integer? (syntax->datum $a)) (integer? (syntax->datum $b)))
            (datum->syntax #'bitwise-ior (bitwise-ior (syntax->datum $a) (syntax->datum $b)))
            #`(bitwise-ior #,$a #,$b))))
      ...
      ((bitwise-xor a b)
        (lets
          ...
          ($b (expand-expr $env #'b))
          (if (and (integer? (syntax->datum $a)) (integer? (syntax->datum $b)))
            (datum->syntax #'bitwise-xor (bitwise-xor (syntax->datum $a) (syntax->datum $b)))
            #`(bitwise-xor #,$a #,$b))))
      ...
      ((bitwise-not a)
        (lets
          ($a (expand-expr $env #'a))
          (if (integer? (syntax->datum $a))
            (datum->syntax #'bitwise-not (bitwise-not (syntax->datum $a)))
            #`(bitwise-not #,$a))))
      ((bitwise-arithmetic-shift-left a b)
        (lets
          ($a (expand-expr $env #'a))
          ($b (expand-expr $env #'b))
          (if (and (integer? (syntax->datum $a)) (integer? (syntax->datum $b)))
            (datum->syntax #'bitwise-arithmetic-shift-left (bitwise-arithmetic-shift-left (syntax->datum $a) (syntax->datum $b)))
            #`(bitwise-arithmetic-shift-left #,$a #,$b))))
      ((bitwise-arithmetic-shift-right a b)
        (lets
          ($a (expand-expr $env #'a))
          ($b (expand-expr $env #'b))
          (if (and (integer? (syntax->datum $a)) (integer? (syntax->datum $b)))
            (datum->syntax #'bitwise-arithmetic-shift-right (bitwise-arithmetic-shift-right (syntax->datum $a) (syntax->datum $b)))
            #`(bitwise-arithmetic-shift-right #,$a #,$b))))
      ((ref var x ...)
        #`(ref
          #,(expand-expr $env #'var)
