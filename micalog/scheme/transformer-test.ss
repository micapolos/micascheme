...
(check-scheme (expr (%= 6 a b)) (if (= a b) 1 0))
(check-scheme (expr (%!= 6 a b)) (if (not (= a b)) 1 0))
(check-scheme (expr (%< 6 a b)) (if (< a b) 1 0))
(check-scheme (expr (%<= 6 a b)) (if (<= a b) 1 0))
(check-scheme (expr (%> 6 a b)) (if (> a b) 1 0))
(check-scheme (expr (%>= 6 a b)) (if (>= a b) 1 0))
...
(check-scheme (expr (%and 1 a b)) (bitwise-and a b))
(check-scheme (expr (%or 1 a b)) (bitwise-ior a b))
...
(check-scheme (expr (%nand 1 a b)) (bitwise-and (bitwise-not (bitwise-and a b)) 1))
(check-scheme (expr (%nor 1 a b)) (bitwise-and (bitwise-not (bitwise-ior a b)) 1))
(check-scheme (expr (%xnor 1 a b)) (bitwise-and (bitwise-not (bitwise-xor a b)) 1))
...
(check-scheme (expr (%not 1 a)) (bitwise-and (bitwise-not a) 1))
(check-scheme (expr (%not 6 a)) (bitwise-and (bitwise-not a) #x3f))

(check-scheme (expr (%if 6 a b c)) (if (= a 1) b c))
...
(check-scheme
  ...
  (run
    (cond
      ((= foo 1) (lets (run (set! foo bar)) (void)))
...
(check-scheme
  ...
  (run
    (cond
      ((= foo 1) (lets (run (set! foo bar)) (void)))
      ((= bar 1) (lets (run (set! zoo zar)) (void))))))
...
(check-scheme
  (module
   ...
   (eval (quote (lets (clock 0) (reset-counter 64) (reset? 1) (run (do () ((and (= reset? 0) (= (bitwise-and (bitwise-not %reset?) 1) 1)) (void)) (lets (run (set! clock (bitwise-xor clock 1))) (run (cond ((= reset-counter 0) (set! reset? 0)) (else (set! reset-counter (- reset-counter 1))))) (run) (void)))) (void))) (environment (quote (micascheme)))))
