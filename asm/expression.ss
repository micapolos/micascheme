(library (asm expression)
  ...
  (define (syntax->expression $locals $syntax)
    (syntax-case $syntax (+ - u8 u16)
      ...
      ((u8 arg)
        (lets
          ($expression (syntax->expression $locals #'arg))
          (expression-with-syntax $expression
            #`(u8 #,(expression-syntax $expression)))))
      ((u16 arg)
        (lets
          ($expression (syntax->expression $locals #'arg))
          (expression-with-syntax $expression
            #`(u16 #,(expression-syntax $expression)))))
      ((+ arg ...)
        (lets
