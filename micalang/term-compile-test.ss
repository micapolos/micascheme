...
(check-compiles (variable 'zero?) zero?)
(check-compiles (application (variable 'zero?) (native 1)) (zero? 1))
(check-compiles (abstraction 'x (native 'int) (lambda (x) x)) (lambda (x) x))
