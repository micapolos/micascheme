    ...
    (switch-exhaustive $term
      ...
      ((variable? $variable)
        (switch (list-ref $env (variable-index $variable))
          ...
          ((else $other) $other)))
      ((abstraction? $abstraction)
        (abstraction (normalize (cons hole $env) (abstraction-body $abstraction))))
      ((application? $application)
        (lets
          ($abstraction (normalize $env (application-lhs $application)))
          ($rhs (normalize $env (application-rhs $application)))
          (normalize (cons $rhs $env) (abstraction-body $abstraction))))))
)
